...
```js
    db.user.insertOne({username:'dandan'});
    db.user.insertMany([{"username": 'dandan'}, {'username': 'DDboom'}]);
```
...
```js
    ...
    db.user.updateOne({name:'dandan'},{$set:{age:18}})
...
* 正则表达式
```js
    db.user.find({"name":/dd/i});//查找name属性包含jack的数据（不区分大小写）
```
...
* 恢复
    ...
    * 参数或名：
    ...
    ```bash
        mongorestore -h 192.168.0.1:27017 -d mytest --dir D:/mongodump/
    ```
